...
class MapFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, state: Bundle?) {
        ...
        // User kartına basanda aşağı panel açılır və kart özü gizlənir ki, panelin üstünə düşməsin.
        b.mapUserCard.setOnClickListener {
            b.mapUserCard.visibility = View.INVISIBLE
            sheet.state = BottomSheetBehavior.STATE_EXPANDED
        }
        sheet.addBottomSheetCallback(object : BottomSheetBehavior.BottomSheetCallback() {
            override fun onStateChanged(bottomSheet: View, newState: Int) {
                b.mapUserCard.visibility = if (newState == BottomSheetBehavior.STATE_COLLAPSED) View.VISIBLE else View.INVISIBLE
            }

            override fun onSlide(bottomSheet: View, slideOffset: Float) {
                if (slideOffset > 0.02f) b.mapUserCard.visibility = View.INVISIBLE
            }
        })
        ...
        b.zoomIn.setOnClickListener { b.map.controller.zoomIn() }
        b.zoomOut.setOnClickListener { b.map.controller.zoomOut() }
    ...
    private fun addGpsMarkers(data: List<LocationPoint>) {
        ...
        for (i in data.indices step step) {
            shown.add(i)
            addGpsMarker(data[i], i)
        }
        if (data.isNotEmpty() && data.lastIndex !in shown) addGpsMarker(data.last(), data.lastIndex)
    }

    private fun addGpsMarker(x: LocationPoint, index: Int) {
        b.map.overlays.add(Marker(b.map).apply {
            position = gp(x)
            title = "GPS nöqtəsi ${index + 1}"
            snippet = "${displayLocationDateTime(x.recorded_at)} • Dəqiqlik ${x.accuracy_m ?: "—"} m • Batareya ${x.battery_pct ?: "—"}%"
            setAnchor(Marker.ANCHOR_CENTER, Marker.ANCHOR_CENTER)
            // Görünən GPS nöqtəsi 15dp-dir, amma toxunma sahəsi rahat qalır.
            icon = gpsDot15dp()
        })
    }
    ...
    private fun gpsDot15dp(): BitmapDrawable {
        val density = resources.displayMetrics.density
        val touchSize = (32f * density).roundToInt().coerceAtLeast(32)
        val dotSize = (15f * density).roundToInt().coerceAtLeast(15)
        val bmp = Bitmap.createBitmap(touchSize, touchSize, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(bmp)
        val cx = touchSize / 2f
        val cy = touchSize / 2f
        val outerRadius = dotSize / 2f
        val white = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.WHITE }
        val blue = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.parseColor("#2478F3") }
        canvas.drawCircle(cx, cy, outerRadius, white)
        canvas.drawCircle(cx, cy, (outerRadius - 2f * density).coerceAtLeast(2f), blue)
        return BitmapDrawable(resources, bmp)
    }
